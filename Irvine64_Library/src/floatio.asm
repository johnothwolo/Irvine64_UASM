...
;---------------------------------------------------------
ReadFloat PROC USES rax rbx rcx
...
;--------------------------------------------------------------
WriteFloat PROC USES rax rcx rdx
;
...
W2a:    
    ...
    jl   W4
    xor  rdx,rdx    	; it's > 10^9
    add  rax,5      	; for rounding
...
;--------------------------------------------------------
power10  PROC uses rbx rcx r9
;
; power10 expects:  RAX (signed exponent)
; This returns 10.0^(sign*RAX) in the FPU
;--------------------------------------------------------
.data
...
.code
    lea r9, binpwr10
      ...
      .IF (sqword ptr rax > 99) ; MAXEXPONENT
        ; complain
        lea  rdx, ErrMsg
        call WriteString
        ; ...but return 1.0
      ...
      ; The general idea is that if rax= 11101b, then the value wanted
      ;  is 10^(11101b)= 10^(2^4) * 10^(2^3) * 10^(2^2) * 10^(2^0)
      ; So we use a table of these powers, binpwr10.
      ; we start with 10^1
      fld1
      mov  rcx, TOPPWR
	  mov  rsi,(type binpwr10)*(TOPPWR-1)
    P1:
      test  rax,1
      jz    P2
      fmul  real8 ptr [r9+rsi]
    P2:
	  sub   rsi,type binpwr10
      shr   rax,1
      loopnz  P1
      .IF (bl != 0)
        ; take the reciprocal
...
S2: ; here, it's > 10
    ...
    mov  rcx,TOPPWR
    mov  rsi,0    	; index to binpwr10
    mov  rdi,0    	; index to bpwr10
S2a:
    fcom dword ptr [r9+rsi]
    call fcompare
    jl   S2c
    fdiv dword ptr [r9+rsi]
	mov  ax, [r9+rdi]
    add  exponent,ax
S2c:
    add  rsi,type binpwr10
    add  rdi,type bpwr10
    loop S2a
    jmp  S4
...
S3: ; here, it's < 1.0
    mov  rcx,TOPPWR
    mov  rsi,0    	; index to binpwrM10
    mov  rdi,0    	; index to mpwr10
S3a:
    fcom real8 ptr [r10+rsi]
    call fcompare
    jge  S3c
    fdiv real8 ptr [r10+rsi]
	mov  ax, [r9+rdi]
    sub  exponent,ax
S3c:
    add  rsi,type binpwr10
...
;
; (Helper procedure) Writes 'rcx' digits of register rax
